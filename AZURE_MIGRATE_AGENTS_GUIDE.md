...
> |---|---|---|---|---|
> | `sessionId` | Text | GUID generated by the Compose action | `""` | `coalesce(outputs('Generate_Session_ID'), '')` |
...
> | `downloadUrl` | Text | SAS URL or sharing link for the uploaded/generated file | `""` | `coalesce(body('Create_SAS_URI_by_path_(V2)')?['WebUrl'], '')` |
>
> **Note:** The `status` and `message` outputs use literal string values in this flow, so `coalesce()` is not required for them. Use `coalesce()` only for dynamic expressions that reference action outputs (such as `sessionId` and `downloadUrl`) where a preceding action could return null.
>
...
> **Important:** Every output in the **Respond to the agent** action must have a value. Wrap dynamic expressions in `coalesce()` (e.g., `coalesce(body('Create_SAS_URI_by_path_(V2)')?['WebUrl'], '')`) so the flow returns an empty string instead of null when an action produces no result. If the **Create SAS URI by path (V2)** action is not available in your environment, use an Azure Function to generate SAS URLs (see the [Generating SAS URLs for Download](#generating-sas-urls-for-download) section below) and reference its output instead.
>
